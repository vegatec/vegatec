...
export class BusinessComponent implements OnInit {
  ...
  trackId = (_index: number, item: IBusiness): number => this.businessService.getBusinessIdentifier(item);
  ...
  protected fillComponentAttributesFromResponseBody(data: IBusiness[] | null): IBusiness[] {
    ...
    if ('prev' in this.links) {
      const businessesNew = this.businesses ?? [];
      if (data) {
        for (const d of data) {
          if (businessesNew.map(op => op.id).indexOf(d.id) === -1) {
            businessesNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    if (this.term) queryObject['name.contains'] = this.term;
