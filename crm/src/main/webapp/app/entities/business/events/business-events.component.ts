...
import { AddProductsDialogComponent } from '../add-products/add-products-dialog.component';
import { ITEM_ADDED_EVENT } from '../../../config/navigation.constants';
import { BusinessService } from '../service/business.service';

@Component({
  standalone: true,
  selector: 'jhi-business-events',
...
export class BusinessEventsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.parentSubscription = this.activatedRoute.parent?.params.subscribe(params => {
      this.businessId = +params['id'];
      this.load();
    });
  }

  delete(e: any, product: IProduct): void {
    this.businessService.removeProduct(this.businessId, product.id).subscribe(r => this.reset());
  ...
  addProducts(): void {
    const modalRef = this.modalService.open(AddProductsDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.businessId = this.businessId;
    // unsubscribe not needed because closed completes on modal close
    modalRef.closed
      .pipe(
        filter(reason => reason === ITEM_ADDED_EVENT),
        switchMap(() => this.loadFromBackendWithRouteInformations()),
      )
      .subscribe({
        next: (res: EntityArrayResponseType) => {
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    if (this.businessId) queryObject['locatedAtId.equals'] = this.businessId;
