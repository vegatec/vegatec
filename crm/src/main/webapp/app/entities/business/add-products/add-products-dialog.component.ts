...
import { BusinessService, EntityArrayResponseType } from '../service/business.service';
...
export class AddProductsDialogComponent {
  ...
  confirmAdd(): void {
    this.businessService.addProduct(this.businessId!, this.selectedProductId).subscribe(() => {
  ...
  //   if (!this.isSelected(productId)) this.selectedProductIds.push(productId);
  //   else this.selectedProductIds = this.selectedProductIds.filter(id => id !== productId);
  // }

  // isSelected(productId: number): boolean {
  //   return this.selectedProductIds.findIndex(id => id === productId) !== -1;
  // }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    if (this.term) queryObject['id.equals'] = +this.term;
